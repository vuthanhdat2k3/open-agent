...
@pytest.mark.asyncio
async def test_zero_row_org_lists_all_blueprints(test_env):
    db = test_env["db"]
    ...
    assert len(serialized) == len(SYSTEM_AGENT_BLUEPRINTS)
    for s in serialized:
...
async def test_get_by_id_and_key_resolver(test_env):
    ...
    assert agent_by_id.name == "Software & Data Engineer"
...
async def test_fork_on_write_when_updating_system_blueprint(test_env):
    ...
    assert len(org1_agents) == len(SYSTEM_AGENT_BLUEPRINTS)
    ...
    assert len(org2_agents) == len(SYSTEM_AGENT_BLUEPRINTS)
...
async def test_reset_to_template_deletes_override_row(test_env):
    ...
    assert len(agents_after) == len(SYSTEM_AGENT_BLUEPRINTS)
