  ...
  if (notifications.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card/60 p-8 text-center">
        ...
        <p className="mt-3 text-sm font-semibold text-foreground">{locale === "vi" ? "All Caught Up" : "All Caught Up"}</p>
        <p className="mt-1 text-xs text-muted-foreground">
          {locale === "vi" ? "No new emails requiring classification or action." : "No new emails requiring classification or action."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex items-center justify-between font-mono text-[10.5px] text-muted-foreground">
        <span>{locale === "vi" ? "INBOUND EMAIL TRIAGE (" : "INBOUND EMAIL TRIAGE ("}{notifications.length} {locale === "vi" ? "NEW)" : "NEW)"}</span>
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              disabled={page === 0}
              title={locale === "vi" ? "Previous 5 items" : "Previous 5 items"}
            >
            ...
            <Button
              ...
              title={locale === "vi" ? "Next 5 items" : "Next 5 items"}
            >
                ...
                <div className="border-t border-border/60 bg-muted/40 p-3.5 text-xs">
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-border/50 pt-2.5 text-[11px]">
                    ...
                    {onOpenEmail && (
                      ...
                      >
                        {locale === "vi" ? "Inspect Full Email" : "Inspect Full Email"}<ExternalLink className="h-3 w-3" />
                      </Button>
                    )}
                  </div>
                </div>
      ...
      <div className="flex justify-center pt-1">
        ...
        >
          {locale === "vi" ? "View All" : "View All"}{notifications.length} {locale === "vi" ? "Emails in Smart Inbox (/email-intelligence)" : "Emails in Smart Inbox (/email-intelligence)"}<ArrowRight className="h-3 w-3" />
        </Link>
      </div>
    </div>
  );
...
export function BriefingsAccordion({
  ...
  if (cases.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card/60 p-8 text-center">
        ...
        <p className="mt-3 text-sm font-semibold text-foreground">{locale === "vi" ? "No Briefings Available" : "No Briefings Available"}</p>
        <p className="mt-1 text-xs text-muted-foreground">
          {locale === "vi" ? "New company dossiers and meeting briefs will appear here after automated synthesis." : "New company dossiers and meeting briefs will appear here after automated synthesis."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {/* Header with 5-item Pagination Controls */}
      <div className="flex items-center justify-between font-mono text-[10.5px] text-muted-foreground">
        <span>{locale === "vi" ? "BRIEFINGS & DOSSIERS (" : "BRIEFINGS & DOSSIERS ("}{cases.length} {locale === "vi" ? "READY)" : "READY)"}</span>
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
            <span>
              {page + 1} / {totalPages}
        ...
        {currentPageItems.map((c) => (
          ...
          >
            ...
            <div className="flex items-center gap-1 text-[11px] font-medium text-primary">
              <span>{locale === "vi" ? "View Dossier" : "View Dossier"}</span>
      ...
      <div className="flex justify-center pt-1">
        <Link
          ...
          className="flex items-center gap-1 font-mono text-[11px] text-muted-foreground hover:text-primary hover:underline transition-colors"
        >
          {locale === "vi" ? "View All" : "View All"}{cases.length} {locale === "vi" ? "Dossiers in Client Intelligence (/customer-intelligence)" : "Dossiers in Client Intelligence (/customer-intelligence)"}<ArrowRight className="h-3 w-3" />
        </Link>
      </div>
    </div>
  );
