    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card/60 p-8 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          No new emails requiring classification or action.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex items-center justify-between font-mono text-[10.5px] text-muted-foreground">
        <span>INBOUND EMAIL TRIAGE ({notifications.length} NEW)</span>
        {totalPages > 1 && (
                ...
                <div className="border-t border-border/60 bg-muted/40 p-3.5 text-xs">
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-border/50 pt-2.5 text-[11px]">
                    ...
                    {onOpenEmail && (
                      ...
                      >
                        Inspect Full Email <ExternalLink className="h-3 w-3" />
                      </Button>
                    )}
                  </div>
                </div>
      ...
      <div className="flex justify-center pt-1">
        ...
        >
          View All {notifications.length} Emails in Smart Inbox (/email-intelligence) <ArrowRight className="h-3 w-3" />
        </Link>
      </div>
    </div>
  );
...
export function BriefingsAccordion({
  ...
  if (cases.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card/60 p-8 text-center">
        ...
        <p className="mt-3 text-sm font-semibold text-foreground">No Briefings Available</p>
        <p className="mt-1 text-xs text-muted-foreground">
          New company dossiers and meeting briefs will appear here after automated synthesis.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {/* Header with 5-item Pagination Controls */}
      <div className="flex items-center justify-between font-mono text-[10.5px] text-muted-foreground">
        <span>BRIEFINGS & DOSSIERS ({cases.length} READY)</span>
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
            <span>
              {page + 1} / {totalPages}
      ...
      <div className="flex justify-center pt-1">
        <Link
          ...
          className="flex items-center gap-1 font-mono text-[11px] text-muted-foreground hover:text-primary hover:underline transition-colors"
        >
          View All {cases.length} Dossiers in Client Intelligence (/customer-intelligence) <ArrowRight className="h-3 w-3" />
        </Link>
      </div>
    </div>
  );
