...
> 2026-09-07: Đã merge PR #315 vào `dev` và deploy lên `deploy/dev`: Cô lập hoàn toàn message của Sub-agent trong `backend/app/core/agent_loop.py` (thêm điều kiện `depth == 0` cho `defer_user_message`, `slog.append_event(USER_MESSAGE)`, `derive_messages`, `slog.append_event(ASSISTANT_MESSAGE)` và `_persist` ngăn sub-agent sinh bong bóng chat user/assistant giả mạo trên web); sửa triệt để crash-loop của `worker` và `classification-worker` trong `docker-compose.yml` do thiếu biến `OPENAGENT_COOKIE_SECURE` và `OPENAGENT_S3_*` khi `OPENAGENT_RUNTIME=production`; cải tiến `_run_agent_node` trong `backend/app/core/workflow/engine.py` tự động phân giải Tên model sang Model UUID active; bổ sung unit tests `test_subagent_message_isolation.py`; live-test thành công workflow "Fast Data Filter" trả về kết quả `[12, 8, 21, 7]` và giao diện chat hiển thị gọn gàng.

> 2026-09-07: Đã merge PR #306, #309 vào `dev` và deploy lên `deploy/dev` qua PR #307, #310: Bổ sung System Administrator Agent (`system-admin`) làm System Blueprint toàn cục cho mọi Organization kèm bộ 20 công cụ platform management (`backend/app/core/tools/system_admin.py`). Bao gồm 5 nhóm tính năng: (1) Quản lý Providers (list, get, templates, create, update, test connection); (2) Quản lý Models (list, get, toggle active, update metadata, test chat); (3) Model Tier Routing Matrix (get/set economy, balanced, frontier model bindings); (4) Quản lý Quotas (get usage, set monthly cost, request rates, run limits); (5) Quản lý Agents Nhóm A (liệt kê agents kèm model đang dùng, xem chi tiết, đổi trực tiếp model cho agent khi lỗi quota `system_update_agent_model`, cập nhật cấu hình, reset về blueprint mặc định `system_reset_agent`). Đã khắc phục lỗi async lazy load `m.provider` trên PostgreSQL asyncpg bằng prefetching providers map, cấu hình `visibility="all"` cho phép người dùng tương tác trực tiếp trên giao diện web (/agents). Đã live-test thành công toàn diện các công cụ trên container API thật và qua giao diện trình duyệt.
